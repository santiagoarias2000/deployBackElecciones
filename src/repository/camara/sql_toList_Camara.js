...
exports.SQL_TO_LIST_CAMARA = {
    LIST_CAMARA_DEPARTAMENTO_TERRITORIAL: "SELECT dp.name_department, SUM(vs.number_votes) AS votos,  \
...
ORDER BY dp.name_department ASC",
    LIST_CAMARA_DEPARTAMENTO_INDIGENA: "SELECT dp.name_department, SUM(vs.number_votes) AS votos,  \
    jsonb_build_object('idDepartment', dp.id_department, 'name_department', dp.name_department) AS department \
    FROM votes vs \
INNER JOIN department dp ON dp.id_department = vs.id_department \
...
GROUP BY dp.id_department, dp.name_department \
ORDER BY dp.name_department ASC",
    LIST_CAMARA_DEPARTAMENTO_AFRO_DESCENDIENTE: "SELECT dp.name_department, SUM(vs.number_votes) AS votos,  \
    jsonb_build_object('idDepartment', dp.id_department, 'name_department', dp.name_department) AS department \
    FROM votes vs \
INNER JOIN department dp ON dp.id_department = vs.id_department \
