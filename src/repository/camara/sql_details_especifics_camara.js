...
exports.SQL_DETAILS_ESPECIFICS_CAMARA = {
    CIRCUNCRIPCION_TERRITORIAL_CAMARA_DEPARTAMENTO: "SELECT vs.candidate_name, vs.description_politicparty, SUM(vs.number_votes) AS VOTOS, t1.VOTOS AS VOTOSSSS, \
...
ORDER BY t1.VOTOS DESC, VOTOS DESC",
    CIRCUNCRIPCION_INDIGENA_CAMARA_DEPARTAMENTO: "SELECT vs.candidate_name, vs.description_politicparty, SUM(vs.number_votes) AS VOTOS, t1.VOTOS AS VOTOSSSS, \
    jsonb_object(array['idDepartment',dp.id_department,'name_department',dp.name_department]::TEXT[]) AS department \
FROM votes vs \
INNER JOIN department dp ON dp.id_department = vs.id_department \
INNER JOIN ( \
...
GROUP BY vs.candidate_name, vs.description_politicparty, dp.id_department, t1.VOTOS \
ORDER BY t1.VOTOS DESC, VOTOS DESC",
    CIRCUNCRIPCION_AFRODESCENDIENTE_CAMARA_DEPARTAMENTO: "SELECT vs.candidate_name, vs.description_politicparty, SUM(vs.number_votes) AS VOTOS, t1.VOTOS AS VOTOSSSS, \
    jsonb_object(array['idDepartment',dp.id_department,'name_department',dp.name_department]::TEXT[]) AS department \
FROM votes vs \
INNER JOIN department dp ON dp.id_department = vs.id_department \
INNER JOIN ( \
...
GROUP BY vs.candidate_name, vs.description_politicparty, dp.id_department, t1.VOTOS \
ORDER BY t1.VOTOS DESC, VOTOS DESC",
    CIRCUNCRIPCION_TERRITORIAL_CAMARA_DEPARTAMENTO_MUNICIPIO: "SELECT vt.candidate_name,vt.description_politicparty,sum(vt.number_votes) as VOTOS, t3.votos, t4.votos AS votos_muicipio, \
    ...
    CIRCUNCRIPCION_INDIGENA_CAMARA_DEPARTAMENTO_MUNICIPIO: "SELECT vt.candidate_name, vt.description_politicparty, sum(vt.number_votes) AS VOTOS, t3.votos, \
    ...
    WHERE t3.code_role = 2 AND t3.code_district = 4 AND t3.id_department = $1 \
    ...
    ORDER BY t1.VOTOS DESC, t4.votos DESC",
    CIRCUNCRIPCION_AFRODESCENDIENTE_CAMARA_DEPARTAMENTO_MUNICIPIO: "SELECT vt.candidate_name, vt.description_politicparty, sum(vt.number_votes) AS VOTOS, t3.votos, \
    CASE WHEN t4.votos IS NULL THEN '0' ELSE t4.votos END AS votos_muicipio, \
    jsonb_object(array['idDepartment', t3.id_department, 'name_department', t3.name_department]::TEXT[]) AS department, \
    jsonb_object(array['idMunicipality', t4.id_municipality, 'name_municipality', t4.name_municipality]::TEXT[]) AS municipality \
    FROM t3 \
    ...
    WHERE t3.code_role = 2 AND t3.code_district = 5 AND t3.id_department = $1 \
    GROUP BY vt.candidate_name, vt.description_politicparty, t3.id_department, t3.name_department, t4.id_municipality, \
        t4.name_municipality, t3.votos, t4.votos, t1.VOTOS \
    ORDER BY t1.VOTOS DESC, t4.votos DESC ",
};
